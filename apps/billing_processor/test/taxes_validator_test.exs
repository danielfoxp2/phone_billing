defmodule BillingProcessor.TaxesValidatorTest do
  ...
  describe "taxes" do
    test "should invalidate when reference period don't exists" do
      ...
      expected_result_for_taxes_with_nil_reference_period = %{"reference_period" => nil, "errors" => [error_message]}
      expected_result_for_taxes_with_empty_reference_period = %{"reference_period" => "", "errors" => [error_message]}
      expected_result_for_taxes_without_reference_period = %{"errors" => [error_message]}

      assert TaxesValidator.validate(taxes_with_nil_reference_period) == expected_result_for_taxes_with_nil_reference_period
      assert TaxesValidator.validate(taxes_with_empty_reference_period) == expected_result_for_taxes_with_empty_reference_period
      assert TaxesValidator.validate(taxes_without_reference_period) == expected_result_for_taxes_without_reference_period
    end
    ...
    test "should invalidate when reference period has not MM/AAAA format" do
      ...
      assert TaxesValidator.validate(params_with_invalid_reference) == expected_result_for_invalid_reference
    end
